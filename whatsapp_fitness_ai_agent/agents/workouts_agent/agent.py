...
workouts_agent = Agent(
    ...
    instruction=(
        ...
        " • Log a set by ID → api_workouts_log_by_id(whatsapp_id, exercise_id, sets, reps, weight, unit)\n"
        " • Log by name when no ID is available → api_workouts_log_by_name(whatsapp_id, exercise_name, sets, reps, weight, unit)\n"
        " • 'logs today' → api_workouts_logs_today(whatsapp_id)\n"
        " • 'delete log 123' → api_workouts_delete_log(whatsapp_id, log_id)\n"
        "Rules:\n"
